...
$(document).ready(function () {
    ...
    if (orderNo != '') {
        ...
        handelAjax('OrderAdd.aspx/GetOrderInfo', 'Post', { c_no: orderNo }, function (data) {
    ...
    $("#selectTechProt").change(function () {
        ...
        handelAjax('OrderAdd.aspx/GetUSES', 'Post', { C_STD_ID: STD_ID }, function (data) {
    ...
    $("#txtNum").change(function () {
        ...
        handelAjax('OrderAdd.aspx/GetTime', 'Post', { stlgrd: stlgrd, spec: spec, stdcode: protNo, num: num }, function (data) {
            if (data.d != '') {
                var time = data.d;
                $("#txtDate").val(time);
                $("#hidSysDate").val(time);
...
function SetMat(code, name, grd, spec, type, dia) {
    ...
    if (grd != "") {
        ...
        handelAjax('OrderAdd.aspx/GetCUST_TECH_PROT', 'Post', { stl_grd: grd }, function (data) {
            data = jQuery.parseJSON(data.d);
            $.each(data, function (i, item) {
                $("#selectTechProt").append("<option value='" + item.C_STD_ID + "'>" + item.C_CUST_TECH_PROT + "</option>");
                if (i == 0) {
                    handelAjax('OrderAdd.aspx/GetUSES', 'Post', { C_STD_ID: item.C_STD_ID }, function (data) {
...
function SavaData() {
    ...
    handelAjax('OrderAdd.aspx/AddOrder', 'Post', parm, function (data) {
