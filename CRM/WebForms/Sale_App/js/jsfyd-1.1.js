...
function save() {
    ...
    var zd = $("#dropfyfs");
    var zdnr = $("#txtdz");
    var planno = $("#txtplanno");
    if (zd.val() == "0001NC10000000003ILQ" && $.trim(planno.val()) == "") {
        alert("火运计划号不能为空，需要生成火运计划请勾选！");
        return false;
    }
    ...
    if (zd.val() == "0001NC10000000003ILR" && sfbdj == "0") {
        alert("是否包到请选择！");
        return false;
    }
    ...
    if (sfbdj == "0") {
        ...
        var xnch = $.trim($("#txtxnch").val());
        if (xnch == "") {
            alert("请输入虚拟车号");
            return false;
        }
        if (isVehicleNumber(xnch) == false) {
            alert("请输入正规有效虚拟车号");
            return false;
        }
...
function openWeb(index) {
    switch (index) {
        ...
        case 3:
            ...
            break;
        case 4:
            var url = "../Train/train_plan_view.aspx";
