    ...
    public partial class Dal_Interface_NC_DM
    {
        ...
        public string SendXml_DM(string xmlFileName, string fydh)
        {
            try
            {
                ...
                root.SetAttribute("filename", fydh);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        Mod_TMD_DISPATCHDETAILS moditem = dal_fyditem.GetModel(dt.Rows[i]["C_PK_NCID"].ToString());

                        Mod_TMP_DAYPLAN modDayPlan = dal_TMP_DAYPLAN.GetModelBYID(dt.Rows[i]["C_PLAN_ID"].ToString());
                        if (modDayPlan != null)
                        {
                            #region //实体参数
                            ...
                            {
                                DataTable dtname = dal_TMB_AREA.GetAddrNCPK(moditem.C_AOG_SITE).Tables[0];
                            ...
                            string wgt = modfyditem.N_FYWGT.ToString();//重量
