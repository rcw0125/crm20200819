//------------------------------------------------------------------------------
// <auto-generated>
//    此代码是根据模板生成的。
//
//    手动更改此文件可能会导致应用程序中发生异常行为。
//    如果重新生成代码，则将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace NF.EF.Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class TPB_LGJH
    {
        public string C_ID { get; set; }
        public short N_STATUS { get; set; }
        public string C_EMP_ID { get; set; }
        public System.DateTime D_MOD_DT { get; set; }
        public string C_REMARK { get; set; }
        public string C_STD_CODE { get; set; }
        public string C_STL_GRD { get; set; }
        public string C_ZYX1 { get; set; }
        public string C_ZYX2 { get; set; }
        public string C_STEEL_SIGN { get; set; }
        public string C_GRADE_GROUP_CODE { get; set; }
        public string C_FLRST_FLAG { get; set; }
        public string C_USE_FLAG { get; set; }
        public string C_STEELGRADE_TYPE { get; set; }
        public string C_NAME { get; set; }
    }
}
