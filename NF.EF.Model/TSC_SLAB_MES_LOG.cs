//------------------------------------------------------------------------------
// <auto-generated>
//    此代码是根据模板生成的。
//
//    手动更改此文件可能会导致应用程序中发生异常行为。
//    如果重新生成代码，则将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace NF.EF.Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class TSC_SLAB_MES_LOG
    {
        public string C_ID { get; set; }
        public string C_STA_DESC { get; set; }
        public string C_STOVE { get; set; }
        public string C_STL_GRD { get; set; }
        public string C_STD_CODE { get; set; }
        public string C_SPEC { get; set; }
        public string C_LEN { get; set; }
        public string C_MAT_CODE { get; set; }
        public string C_MAT_NAME { get; set; }
        public string C_QUA { get; set; }
        public string C_WGT { get; set; }
        public Nullable<System.DateTime> D_DEL_TIME { get; set; }
        public string C_DEL_USER { get; set; }
        public string C_REASON { get; set; }
    }
}
