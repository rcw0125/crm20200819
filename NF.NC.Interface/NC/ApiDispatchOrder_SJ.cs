...
namespace NF.NC.Interface
{
    ...
    /// 不锈钢委外
    /// </summary>
    public partial class ApiDispatchOrder_SJ
    {
        ...
        public List<string> SendXmlApiDispatchOrder(string code, string xmlFileName, string empID, int flag)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (modDayPlan != null)
                        {
                            ...
                            CreateNode(xmlDoc, item, "nassistnetwgt", moditem.N_FYZS.ToString());//辅数量
                            CreateNode(xmlDoc, item, "nfeedbacknum", moditem.N_FYWGT.ToString());//回写上游数量
                            CreateNode(xmlDoc, item, "nnetwgt", moditem.N_FYWGT.ToString());// 净重
                            CreateNode(xmlDoc, item, "nquoteunitnum", moditem.N_FYWGT.ToString());//报价计量单位数量？-存货数量
                            ...
                            CreateNode(xmlDoc, item, "vbatchcode", moditem.C_BATCH_NO);//批次号
                            ...
                            CreateNode(xmlDoc, item, "vfree3", modOrder.N_TYPE == 6 ? "" : moditem.C_PACK);//包装8线材
                ...
                //{
                //    //更新发运单状态
                //    tmd_dispatch.UpdateFydStatus(code, 2, "", "", "3");
                //}
                return parem;
            }
            catch (Exception ex)
