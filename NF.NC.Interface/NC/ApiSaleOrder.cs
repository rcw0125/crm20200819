...
namespace NF.NC.Interface
{
    ...
    /// 销售订单导入NC接口
    /// </summary>
    public partial class ApiSaleOrder
    {
        ...
        public List<string> SendXmlOrder(List<ApiOrderDto> orderDto, string xmlFileName, string empID)
        {
            try
            {
                ...
                CreateNode(xmlDoc, head, "cdeptid", modCon.C_DEPTID);//部门ID "1001NC100000008KOR85"
                CreateNode(xmlDoc, head, "cemployeeid", modCon.C_EMPLOYEEID);//业务员ID  "1001NC10000000000TD8"
                CreateNode(xmlDoc, head, "coperatorid", modCon.C_COPERATORID);//制单人"1006AA1000000000NI7B"
                ...
                for (int j = 0; j < dt.Rows.Count; j++)
                {
                    ...
                    if (modOrder != null)
                    {
                        ...
                        CreateNode(xmlDoc, entry, "cfactoryid", modOrder.C_CON_NO);//合同表头主键
                        CreateNode(xmlDoc, entry, "cfreezeid", modOrder.C_ID);//合同表体主键
