...
namespace NF.NC.Interface
{
    ...
    public partial class ApiWL
    {
        ...
        /// <returns></returns>
        public string  SendXmlApiDispatchOrder(string code, string xmlFileName, string empID)
        {
            try
            {
                ...
                CreateNode(xmlDoc, head, "vuserdef0", modDispatch.C_IS_WIRESALE);// 是否线材销售         
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (modDayPlan != null)
                        {
                            ...
                            string guid = dt.Rows[i]["C_DISPATCH_ID"].ToString() + i.ToString();
                            ...
                            decimal num = Convert.ToDecimal(dt.Rows[i]["N_FYWGT"].ToString());//数量
                            ...
                            CreateNode(xmlDoc, item, "ccheckstate_bid", dt.Rows[i]["C_QUALIRY_LEV"].ToString());//质量等级**
                            ...
                            CreateNode(xmlDoc, item, "cquoteunitname", "吨");//吨
                            CreateNode(xmlDoc, item, "creceiptcorp",GetCust(dt.Rows[i]["C_CGC"].ToString()));//收货单位名称
                            CreateNode(xmlDoc, item, "creceiptcorpid", dt.Rows[i]["C_CGC"].ToString());//收货单位主键
                            ...
                            CreateNode(xmlDoc, item, "dinvassist", dt.Rows[i]["N_FYZS"].ToString());//辅数量-件数
                            CreateNode(xmlDoc, item, "dinvnum", dt.Rows[i]["N_FYWGT"].ToString());//存货数量
                            ...
                            CreateNode(xmlDoc, item, "pk_delivbill_b", guid);//发运单表体主键 dt.Rows[i]["C_ID"].ToString()
                            ...
                            CreateNode(xmlDoc, item, "pksendstock", "");//仓库
                            ...
                            CreateNode(xmlDoc, item, "vbatchcode", "");//炉号
                            ...
                            CreateNode(xmlDoc, item, "vdestaddress", moditem.C_AREA);//收货地址
                            CreateNode(xmlDoc, item, "vdestarea", moditem.C_SEND_AREA);//收货地区主键改成名称----
                            ...
                            CreateNode(xmlDoc, item, "vfree3", dt.Rows[i]["C_PACK"].ToString());//包装
                            ...
                            CreateNode(xmlDoc, item, "vinvname",modOrder.C_MAT_NAME);//存货名称
                            ...
                            CreateNode(xmlDoc, item, "vsendaddress", "邢台市");//邢台市
                            CreateNode(xmlDoc, item, "vsendarea", "邢台市区");//邢台市区
                ...
                string result= NCSendXml.WLSendXML(xmlDoc);
                ...
                mod.C_RESULTCODE =result;//返回结果
                mod.C_PKID =code;//主键
                mod.C_EMPID =empID;//操作人ID
                mod.C_FLAG ="发运单-WL";//标识
                mod.C_REMARK = result== "success"? "导入WL成功": "导入WL失败";//备注
