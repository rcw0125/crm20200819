...
namespace NF.NC.Interface
{
    public partial class ApiWL_SJ
    {
        ...
        /// <returns></returns>
        public bool SendXml_DM(string xmlFileName, string code, List<ApiDispDto> xmlList)
        {
            try
            {
                ...
                DataTable dt = tmd_dispatchdetails.GetZJBList(code).Tables[0];
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        Mod_TMD_DISPATCHDETAILS moditem = tmd_dispatchdetails.GetModel(dt.Rows[i]["C_PK_NCID"].ToString());
                        ...
                        if (modDayPlan != null)
                        {
                            ...
                            Mod_TMD_DISPATCHDETAILS modFYD = tmd_dispatchdetails.GetModel(dt.Rows[i]["C_PK_NCID"].ToString());
                            DataRow drStock = tmd_dispatch.GetStock(dt.Rows[i]["C_SEND_STOCK"].ToString());
                            string pkstock = "";//仓库编码
                            if (drStock != null)
                            {
                                pkstock = drStock["C_ID"].ToString();
                            }
                            string addrpk = "";
                            ...
                            string NCCHECKID = string.Empty;//质量等级
                            DataTable dtcheck = tmd_dispatchdetails.GetCheck(dt.Rows[i]["C_ZLDJ"].ToString(), 6).Tables[0];
                            if (dtcheck.Rows.Count > 0)
                            {
                                NCCHECKID = dtcheck.Rows[0]["C_ID"].ToString();
                            }
                            ...
                            decimal N_JZ = Convert.ToDecimal(dt.Rows[i]["N_JZ"]);//净重
                            string wgt = modFYD.N_FYWGT.ToString();// dt.Rows[i]["N_WGT"].ToString();//计划重量
                            string num = dt.Rows[i]["N_NUM"].ToString();//件数
                            ...
                            CreateNode(xmlDoc, item, "vinvcode", modOrder.C_MAT_CODE);
                            ...
                            CreateNode(xmlDoc, item, "vuserdef5", N_JZ.ToString());// 净重
                ...
                {
                    //更新发运单状态与钢坯库存状态
                    tmd_dispatchdetails.UpdateGPStatus(code, 9);
